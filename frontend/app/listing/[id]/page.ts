...
import { useSimilarListings, useTrackInteraction } from '@/hooks/useRecommendations';
...
function ListingDetailContent() {
  ...
  // ==========================================
  // SIMILAR LISTINGS (via recommendation engine)
  // ==========================================
  ...
  useEffect(() => {
    if (listing && user && user.id !== listing.userId) {
      trackInteraction(listingId, 'VIEW').catch(() => {});
    }
  }, [listing, user, listingId, trackInteraction]);
  ...
  const startChat = async () => {
    ...
    try {
      ...
      await trackInteraction(listingId, 'CHAT');
  ...
  const handleShare = async () => {
    try {
      ...
      // Track share
      if (listing) {
        await api.post(`/listings/${listingId}/share`).catch(() => {});
      }
    } catch {
        ...
        {similarLoading ? (
          ...
          <div className="mt-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-900">Annonces similaires</h2>
            </div>
